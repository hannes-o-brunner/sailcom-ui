  ...
  public template: string = `
    <div>
      <div class="collapsed-content" ng-if="!$ctrl.sharedModel.topbarCollapsed">
        ...
        <textarea class="form-control" ng-model="$ctrl.model.tweetContent"></textarea>
        <br/>
        <button ng-disabled="$ctrl.model.tweetContent === ''" ng-click="$ctrl.post()" class="btn btn-primary btn-lg">
          <i class="fa fa-envelope"></i>&nbsp;Post
        </button>
      </div>
      <i ng-click="$ctrl.toggleCollapsed()" class="fa dp-collapse dp-collapse-center" ng-class="{'fa-chevron-up': !$ctrl.sharedModel.topbarCollapsed, 'fa-chevron-down': $ctrl.sharedModel.topbarCollapsed}"></i>
    </div>
`;
